...
class TrialNet1(nn.Module):
    def __init__(self, load_weights=False):
        super(TrialNet1,self).__init__()
        ...
        self.conv9_1=nn.Conv2d(512,512,kernel_size=1,bias=False)    # added
        self.conv9_2=nn.Conv2d(512,512,kernel_size=1,bias=False)    # added
        self.conv12_1=nn.Conv2d(512,512,kernel_size=1,bias=False)    # added
        self.conv12_2=nn.Conv2d(512,512,kernel_size=1,bias=False)    # added
        if not load_weights:
...
#        print('w6',w6.mean())
        ...
        # S=9
        #        print('fv',fv.mean())
        ave9 = nn.functional.adaptive_avg_pool2d(fv, (9, 9))
        #        print('ave9',ave9.mean())
        ave9 = self.conv9_1(ave9)
        #        print(ave9.mean())
        #        ave9=nn.functional.relu(ave9)
        s9 = nn.functional.upsample(ave9, size=(fv.shape[2], fv.shape[3]), mode='bilinear')
        #        print('s9',s9.mean(),'s1',s1.mean(),'s2',s2.mean(),'s3',s3.mean())
        c9 = s9 - fv
        #        print('c9',c9.mean())
        w9 = self.conv9_2(c9)
        w9 = nn.functional.sigmoid(w9)
        #        print('w9',w9.mean())

    # ADDED LAYER
        # S=12
        #        print('fv',fv.mean())
        ave12 = nn.functional.adaptive_avg_pool2d(fv, (12, 12))
        #        print('ave12',ave12.mean())
        ave12 = self.conv12_1(ave12)
        #        print(ave12.mean())
        #        ave12=nn.functional.relu(ave12)
        s12 = nn.functional.upsample(ave12, size=(fv.shape[2], fv.shape[3]), mode='bilinear')
        #        print('s12',s12.mean(),'s1',s1.mean(),'s2',s2.mean(),'s3',s3.mean())
        c12 = s12 - fv
        #        print('c12',c12.mean())
        w12 = self.conv12_2(c12)
        w12 = nn.functional.sigmoid(w12)
        #        print('w12',w12.mean())
    # ADDED UNTIL HERE
        fi=(w1*s1+w2*s2+w3*s3+w6*s6+w9*s9+w12*s12)/(w1+w2+w3+w6+w9+w12+0.000000000001)
#        print('fi',fi.mean())
#        fi=fv
        x=torch.cat((fv,fi),1)
